...
class Herramienta_Proveedor:
    ...
    def alta(self, e):
        ...
        volver_btn = ft.ElevatedButton("Volver", on_click=lambda e: self.mostrar_proveedor())
        self.page.add(self.nombre, self.telefono, self.email, ft.Row([guardar_btn, volver_btn]))
        self.page.update()

    def guardar(self, e):
        try:
            query = "INSERT INTO proveedor (nombre, telefono, email) VALUES (%s, %s, %s)"
            values = (self.nombre.value, self.telefono.value, self.email.value)
            self.cursor.execute(query, values)
            self.connection.commit()
            self.page.snack_bar = ft.SnackBar(ft.Text("Proveedor agregado exitosamente"))
    ...
    def editar(self, id):
        self.page.clean()
        self.cursor.execute("SELECT * FROM proveedor WHERE cod_proveedor = %s", (id,))
        proveedor = self.cursor.fetchone()
        self.nombre = ft.TextField(label="Nombre", value=proveedor[1])
        self.telefono = ft.TextField(label="Teléfono", value=proveedor[2])
        self.email = ft.TextField(label="Email", value=proveedor[3])
    ...
    def actualizar(self, id):
        query = "UPDATE proveedor SET nombre=%s, telefono=%s, email=%s WHERE cod_proveedor=%s"
        values = (self.nombre.value, self.telefono.value, self.email.value, id)
        self.cursor.execute(query, values)
        self.connection.commit()
        self.page.snack_bar = ft.SnackBar(ft.Text("Proveedor actualizado exitosamente"))
        self.page.snack_bar.open = True
        self.consulta(None)

    def eliminar(self, id):
        query = "DELETE FROM proveedor WHERE cod_proveedor=%s"
        self.cursor.execute(query, (id,))
        self.connection.commit()
        self.page.snack_bar = ft.SnackBar(ft.Text("Proveedor eliminado"))
        self.page.snack_bar.open = True
        self.consulta(None)
